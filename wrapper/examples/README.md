...
| [`archive-then-delete-client-adapter.ts`](./archive-then-delete-client-adapter.ts) | `ArchiveThenDeleteAdapter`, `archiveThenDeleteClient` | no | Compile-checked client migration that preserves editable current fields in the replacement-body envelope before delete. |
...
| [`quickstart.ts`](./quickstart.ts)                           | `clockifyDiagnostics`, `client.health()`                     | yes (read-only) / mock-safe      | The [quickstart receipt](../../docs/quickstart-receipt.md)'s three steps as one script: local diagnostics, client construction, then a health probe. Point `CLOCKIFY_BASE_URL` at a mock or sandbox endpoint to run it with no live account. |
...
| [`typed-errors.ts`](./typed-errors.ts)                       | `isClockifyApiError`, `promoteApiError`, `isRateLimitError`, `getRateLimitFromError` | yes (read-only)                  | Three catch-block styles: outer-edge type guard, promote-then-narrow, and direct type-guard narrowing.                                                      |
...
| [`first-health-check.ts`](./first-health-check.ts)          | `clockifyHealth`, `client.projects.list`                     | yes (read-only)                  | Confirm authentication and read access to the pinned workspace before anything else.                                                                         |
