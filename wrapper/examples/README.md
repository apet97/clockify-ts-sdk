...
| [`start-stop-timer.ts`](./start-stop-timer.ts)              | `client.timeEntries.create/stopTimer/delete`                 | yes (write+delete)               | Start a running timer, stop it, then delete it so a partial run leaves no litter.                                                                           |
