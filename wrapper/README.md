...
modules, 184 live operations, idiomatic `client.<resource>.<verb>()`
naming on 172 of 184 operations (93.5%), and dual ESM + CJS.
...
- Pagination — per-resource `iterAll` + page-envelope `iterPages`
    - low-level `paginate` callback iterator;
      ...
      server emits it (15 of the 20 paginated endpoints)
...
Coverage: 172 ops mapped (93.5% of the 184-op live API surface)
...
arrays. 15 of the 20 paginated endpoints emit a `Last-Page` header
...
| Dual build      | `tsc` ESM + `tsc` CJS + per-format smoke verifying 92 public names + 27 subpaths                                                    | `build:smoke`                           |
