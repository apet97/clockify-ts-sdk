...
TypeScript generator, wrapped with a packable npm layout. 33 resource
modules, 175 operations (161 live-verified — see
[spec-confidence](../docs/spec-confidence.md)), idiomatic `client.<resource>.<verb>()`
naming explicitly governed on 149 operations with 26 governed
operationId-derived methods, and dual ESM + CJS. All 175 methods are generated
...
- Pagination — per-resource `iterAll` + page-envelope `iterPages`
    - low-level `paginate` callback iterator;
      ...
      server emits it (live-audited 2026-05-25; see
      `spec/evidence/discrepancies.md` →
      `pagination.last-page-header.live-audit-2026-05-25`)
...
One sub-client per OpenAPI tag (33 modules). Two name shapes
...
Coverage: 175 generated SDK operations = 149 explicitly named + 26 governed
operationId-derived (161 [live-verified](../docs/spec-confidence.md)) across 33
...
arrays. Most paginated endpoints emit a `Last-Page` header the wrapper
consumes as an authoritative end-of-pages signal (the 2026-05-25 live
audit found it on 18 endpoints; the exact set lives in
`spec/evidence/discrepancies.md`, not here, so this prose cannot rot).
