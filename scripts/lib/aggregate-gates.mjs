...
export function evaluateAggregateGates({
    ...
    function analyzeExecution(label, executionSpec, { rootTarget = null, commands = null } = {}) {
        ...
        // builds each target at most once per invocation, so repeated paths
        // inside ONE invocation are coalesced -- but an unexplained second edge
        // is still the redundant-DAG drift this gate exists to catch.
        ...
        for (const [target, paths] of occurrencePaths.entries()) {
            ...
            if ([...byInvocation.values()].some((bucket) => bucket.length > 1)) {
                failures.push(
                    `${executionName}: ${target} is reached by more than one prerequisite path within a single invocation: ${renderedPaths}`,
                );
            }
