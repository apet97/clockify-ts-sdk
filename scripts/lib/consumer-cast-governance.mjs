...
function restArgumentsSubstitution(values) {
    return { [REST_ARGUMENTS_SUBSTITUTION]: [...values] };
}

function isRestArgumentsSubstitution(value) {
    return Boolean(value?.[REST_ARGUMENTS_SUBSTITUTION]);
}
...
function analyzeProgram({
    ...
    function statementDefinitelyAbrupt(statement) {
        if (
            ...
            ts.isAwaitExpression(unwrapExpression(statement.expression))
        ) {
            const awaited = unwrapExpression(statement.expression).expression;
            const expression = unwrapExpression(awaited);
            if (ts.isCallExpression(expression)) {
                const callee = unwrapExpression(expression.expression);
                const promiseMember =
                    ts.isPropertyAccessExpression(callee) &&
                    isGlobalBuiltin(callee.expression, "Promise")
                        ? callee.name.text
                        : null;
                if (promiseMember === "reject") {
                    return true;
                }
                if (
        ...
        function awaitMayResume(awaited) {
            ...
            function completionKinds(statement, depth = 0) {
                ...
                if (ts.isLabeledStatement(statement)) {
                    const states = completionKinds(statement.statement, depth + 1);
                    if (states.delete(jumpState("break", statement))) states.add("normal");
                    return states;
                }
                    ...
                    if (discriminant.known) {
                        const matched = clauses.findIndex(
                            (clause) =>
                                ...
                                staticDiscriminantValue(clause.expression).known &&
                                Object.is(
                                    staticDiscriminantValue(clause.expression).value,
                                    discriminant.value,
                                ),
                        );
                        const fallback = clauses.findIndex(ts.isDefaultClause);
                        starts = [matched >= 0 ? matched : fallback].filter((index) => index >= 0);
                        if (starts.length === 0) return new Set(["normal"]);
                    } else {
                        starts = clauses.map((_, index) => index);
                    ...
                    for (const start of starts) {
                        ...
                        for (const clause of clauses.slice(start)) {
                            for (const child of clause.statements) {
                                const next = new Set(
                                    [...states].filter((state) => state !== "normal"),
                                );
                                if (states.has("normal")) {
                                    for (const state of completionKinds(child, depth + 1)) {
                                        next.add(state);
                                    }
                                }
                                states = next;
                        ...
                        const matchingBreak = jumpState("break", statement);
                        if (states.delete(matchingBreak)) states.add("normal");
                        for (const state of states) combined.add(state);
                    }
                    return combined;
                }
                if (isIterationStatement(statement)) {
                    const states = completionKinds(statement.statement, depth + 1);
                    const matchingBreak = jumpState("break", statement);
                    const matchingContinue = jumpState("continue", statement);
                    const next = new Set(states);
                    if (next.delete(matchingBreak)) next.add("normal");
                    if (next.delete(matchingContinue) && iterationMayTerminate(statement)) {
                        next.add("normal");
                    }
                    if (next.delete("normal") && iterationMayTerminate(statement)) {
                        next.add("normal");
                    }
                    return next;
                }
    ...
    function matchingMemberWrites(
        ...
        let useRoot = unwrapExpression(use);
        while (ts.isPropertyAccessExpression(useRoot) || ts.isElementAccessExpression(useRoot)) {
            useRoot = unwrapExpression(useRoot.expression);
        }
        const concreteThisProjection = useRoot.kind === ts.SyntaxKind.ThisKeyword;
        ...
        return applyOrderedEffectCutoffs(
            [...propertyWrites, ...wildcardPropertyWrites]
                ...
                .filter((write) => {
                    ...
                    const effectiveReceiver = concreteThisProjection
                        ? receiver
                        : (write.boundaryReceiverOverride ?? receiver);
                    return (
                        write.position < beforePosition &&
            ...
            if (ts.isCallExpression(node)) {
                ...
                if (!skipped) {
                    ...
                    if (ts.isIdentifier(unwrapExpression(node.expression))) {
                        ...
                        const conditionalInitializers = (symbol?.declarations ?? [])
                            .filter(ts.isVariableDeclaration)
                            .map((candidate) => candidate.initializer)
                            .filter(
                                (initializer) =>
                                    initializer &&
                                    (ts.isConditionalExpression(unwrapExpression(initializer)) ||
                                        (ts.isBinaryExpression(unwrapExpression(initializer)) &&
                                            [
                                                ts.SyntaxKind.BarBarToken,
                                                ts.SyntaxKind.AmpersandAmpersandToken,
                                                ts.SyntaxKind.QuestionQuestionToken,
                                            ].includes(
                                                unwrapExpression(initializer).operatorToken.kind,
                                            ))),
                            );
                        for (const initializer of conditionalInitializers) {
                            for (const localDeclaration of callableDeclarationsForValue(
                                initializer,
                                node.pos,
    ...
    function parameterSubstitutions(declaration, invocation) {
        ...
        declaration.parameters.forEach((parameter, index) => {
            if (parameter.dotDotDotToken) {
                const symbol = unalias(checker, checker.getSymbolAtLocation(parameter.name));
                if (symbol) {
                    const lists = restArgumentLists(
                        invocation.arguments,
                        index,
                        invocation.node.pos,
                    );
                    substitutions.set(symbol, {
                        ...restArgumentsSubstitution(lists[0] ?? []),
                    });
                }
                return;
            }
            ...
            if (parameter.initializer) {
                const possibilities = argument
                    ? undefinedPossibilities(argument)
                    : { canUndefined: true, canDefined: false };
                if (possibilities.canUndefined && !possibilities.canDefined) {
    ...
    function parameterSubstitutionAlternatives(declaration, invocation) {
        let alternatives = [new Map(invocation.capturedSubstitutions)];
        declaration.parameters.forEach((parameter, index) => {
            if (parameter.dotDotDotToken) {
                const symbol = unalias(checker, checker.getSymbolAtLocation(parameter.name));
                if (symbol) {
                    ...
                    alternatives = bounded(
                        alternatives.flatMap((substitutions) =>
                            lists.map(
                                (values) =>
                                    new Map([
                                        ...substitutions,
                                        [symbol, restArgumentsSubstitution(values)],
                                    ]),
                            ),
                        ),
                        "rest parameter argument alternatives",
                    );
                }
                return;
            }
            ...
            const possibilities = argument
                ...
                : { canUndefined: true, canDefined: false };
            const values = [
                ...(argument && possibilities.canDefined ? [argument] : []),
                ...(parameter.initializer && possibilities.canUndefined
            ...
            alternatives = bounded(
                alternatives.flatMap((substitutions) =>
                    values.map((value) => new Map([...substitutions, [symbol, value]])),
                ),
                "parameter default substitution alternatives",
            );
        });
        const thisReceiver = invocationThisReceiver(invocation.node);
        if (thisReceiver) {
            alternatives.forEach((substitutions) =>
                substitutions.set(THIS_SUBSTITUTION, thisReceiver),
            );
        }
        return alternatives;
    }
    ...
    function restArgumentLists(args, start, beforePosition) {
        ...
        for (const argument of args.slice(start)) {
            ...
            if (ts.isSpreadElement(argument)) {
                ...
                alternatives = spread.lists;
            } else {
                alternatives = [[argument]];
            }
            lists = bounded(
                lists.flatMap((prefix) => alternatives.map((values) => [...prefix, ...values])),
                "rest parameter argument lists",
            );
        }
    ...
    function substituteReceiver(expression, substitutions) {
        ...
        if (ts.isIdentifier(expression)) {
            ...
            if (substitution?.[REST_ARGUMENTS_SUBSTITUTION]) return expression;
            ...
            for (const declaration of symbol?.declarations ?? []) {
                ...
                while (ts.isBindingElement(current)) {
                    ...
                    if (ts.isArrayBindingPattern(pattern)) {
                        if (current.dotDotDotToken) break;
                        steps.unshift({ kind: "array", index: pattern.elements.indexOf(current) });
                    } else if (ts.isObjectBindingPattern(pattern)) {
                        const name = current.propertyName ?? current.name;
                        const names = ts.isComputedPropertyName(name)
                            ? literalPropertyNames(checker.getTypeAtLocation(name.expression))
                            : [name.getText(current.getSourceFile()).replace(/^['"]|['"]$/g, "")];
                        if (current.dotDotDotToken || names?.length !== 1) break;
                        steps.unshift({ kind: "property", names });
                    } else {
                        break;
                    }
                    if (ts.isVariableDeclaration(pattern.parent)) {
                        source = pattern.parent.initializer;
                        break;
                    }
                ...
                if (ts.isIdentifier(value)) {
                    const sourceSymbol = unalias(checker, checker.getSymbolAtLocation(value));
                    value = substitutions.get(sourceSymbol) ?? value;
                }
                let complete = true;
                for (const step of steps) {
                    if (isRestArgumentsSubstitution(value) && step.kind === "array") {
                        value = value[REST_ARGUMENTS_SUBSTITUTION][step.index];
                    } else {
                        const projected = projectedExpressions(value, [step]);
                        value = projected.length === 1 ? projected[0] : null;
                    }
                    if (!value) {
                        complete = false;
                        break;
        ...
        if (ts.isPropertyAccessExpression(expression) || ts.isElementAccessExpression(expression)) {
            if (
                ...
                const symbol = unalias(checker, checker.getSymbolAtLocation(expression.expression));
                const rest = substitutions.get(symbol)?.[REST_ARGUMENTS_SUBSTITUTION];
                const names = computedPropertyNames(expression);
                if (rest && names?.length === 1 && /^\d+$/.test(names[0])) {
                    return rest[Number(names[0])] ?? expression;
                }
            }
            const base = substituteReceiver(expression.expression, substitutions);
            return base;
        }
        return expression;
    }
    ...
    function activeBoundaryUseProjections(
        ...
        function isRuntimeDescriptorGetter(declaration) {
            ...
            if (
                ts.isMethodDeclaration(declaration) &&
                declaration.name?.getText(declaration.getSourceFile()) === "get" &&
                ts.isObjectLiteralExpression(declaration.parent)
            ) {
                descriptor = declaration.parent;
            } else if (
                (ts.isArrowFunction(declaration) || ts.isFunctionExpression(declaration)) &&
                ts.isPropertyAssignment(declaration.parent) &&
                declaration.parent.name.getText(declaration.getSourceFile()) === "get" &&
                ts.isObjectLiteralExpression(declaration.parent.parent)
            ) {
                descriptor = declaration.parent.parent;
            }
        ...
        if (!accessorReadsIndexed && (ts.isGetAccessorDeclaration(owner) || indexRuntimeGetters)) {
            ...
            function receiverHasPriorMemberWrite(receiver) {
                ...
                const names = ts.isPropertyAccessExpression(receiver)
                    ? [receiver.name.text]
                    : computedPropertyNames(receiver);
                if (names?.length !== 1) return false;
                const base = unwrapExpression(receiver.expression);
                return propertyWrites.some((write) => {
                    if (
                        write.position >= receiver.pos ||
                        !write.propertyNames?.includes(names[0])
                    ) {
                        return false;
                    }
                    ...
                    if (ts.isIdentifier(left) && ts.isIdentifier(base)) {
                        return (
                            ...
                            unalias(checker, checker.getSymbolAtLocation(base))
                        );
                    }
                    return left.getText() === base.getText();
                });
            }
            function concreteReachingExpressions(receiver) {
                if (concreteReachingExpressionCache.has(receiver)) {
                    return concreteReachingExpressionCache.get(receiver);
                }
                const values = receiverHasPriorMemberWrite(receiver)
                    ? reachingExpressionValues(
                          receiver,
                          receiver.pos,
                          new Set(),
                    ...
                    : reachingExpressionValues(receiver, receiver.pos);
                const concrete = values
                    .map((value) => value.expression)
                    .filter((value) => value !== receiver);
                concreteReachingExpressionCache.set(receiver, concrete);
                return concrete;
            }
            function localGetterDeclarations(receiver, names = null, ownEnumerableOnly = false) {
                const concrete = concreteReachingExpressions(receiver);
                const receiverValues = concrete.length > 0 ? concrete : [receiver];
                const properties = receiverValues.flatMap((value) => {
                    const receiverType = checker.getApparentType(checker.getTypeAtLocation(value));
                    return names
                        ? names
            ...
            function recordAccessorActivation(getters, activation, receiver) {
                ...
                if (receiver) {
                    const receivers = accessorReceiverByActivation.get(activation) ?? [];
                    if (!receivers.includes(receiver)) {
                        accessorReceiverByActivation.set(activation, [...receivers, receiver]);
                    }
                }
            }
            ...
            function runtimeGetterDeclarations(
                ...
                const sameRuntimeReceiver = (definition, right) => {
                    ...
                    if (ts.isIdentifier(left) && ts.isIdentifier(right)) {
                        return (
                            unalias(checker, checker.getSymbolAtLocation(left)) ===
                            unalias(checker, checker.getSymbolAtLocation(right))
                        );
                    }
            ...
            function receiversThroughBindingDefault(receivers, initializer, label) {
                ...
                const possibilities = receivers.map((receiver) => ({
                    receiver,
                    possibilities: undefinedPossibilities(receiver),
                }));
                return bounded(
                    [
                        ...possibilities
                            .filter(({ possibilities: value }) => value.canDefined)
                            .map(({ receiver }) => receiver),
                        ...(possibilities.length === 0 ||
                        possibilities.some(({ possibilities: value }) => value.canUndefined)
                            ? [initializer]
                            : []),
                    ],
                    label,
            ...
            function recordBindingPatternActivations(pattern, receivers, activation) {
                const excluded = new Set();
                for (const element of pattern.elements) {
                    if (ts.isOmittedExpression(element)) continue;
                    const names = ts.isObjectBindingPattern(pattern)
                        ? bindingElementNames(element)
                    ...
                    for (const receiver of receivers) {
                        if (!element.dotDotDotToken && names?.length) {
                            ...
                            recordAccessorActivation(
                                localGetterDeclarations(receiver, names),
                                activation,
                                receiver,
                            );
                            if (ts.isArrayBindingPattern(pattern)) {
                                recordAccessorActivation(
                                    runtimeGetterDeclarations(receiver, names, activation),
                                    activation,
                                    receiver,
                                );
                            }
                            ...
                            if (ts.isArrayBindingPattern(pattern)) {
                                const numericNames = runtimeGetterDefinitions
                                    .flatMap((definition) => definition.names)
                                    .filter((name) => /^\d+$/.test(name));
                                recordAccessorActivation(
                                    [
                                        ...localGetterDeclarations(receiver).filter((getter) => {
                    ...
                    const step = ts.isObjectBindingPattern(pattern)
                        ? names?.length === 1
                            ? { kind: "property", names }
                            : null
                        : { kind: "array", index: pattern.elements.indexOf(element) };
                    if (!step) continue;
                    const projected = receivers.flatMap((receiver) =>
                        projectedExpressions(receiver, [step]),
                    );
                    if (element.initializer) {
                        bindingDefaultMayRun.set(
                            element.initializer,
                            projected.length === 0 ||
                                projected.some(
                                    (receiver) => undefinedPossibilities(receiver).canUndefined,
                                ),
                        );
                    }
                    if (
                    ...
                    const nestedReceivers = receiversThroughBindingDefault(
                        projected,
                        element.initializer,
                        "nested implicit accessor binding receivers",
                    );
                    recordBindingPatternActivations(element.name, nestedReceivers, activation);
                }
            }
            function recordAssignmentPatternActivations(pattern, receivers, activation) {
                const properties = ts.isObjectLiteralExpression(pattern)
                    ? pattern.properties
                    : pattern.elements;
                const excluded = new Set();
                for (const [index, property] of properties.entries()) {
                    if (ts.isOmittedExpression(property)) continue;
                    const rest = ts.isSpreadAssignment(property) || ts.isSpreadElement(property);
                    const names = ts.isObjectLiteralExpression(pattern)
                        ? rest
                            ? null
                            : property.name &&
                    ...
                    let target = ts.isPropertyAssignment(property)
                        ? property.initializer
                        : ts.isShorthandPropertyAssignment(property)
                          ? property.name
                          : ts.isSpreadAssignment(property) || ts.isSpreadElement(property)
                            ? property.expression
                            : property;
                    let defaultValue = ts.isShorthandPropertyAssignment(property)
                        ? property.objectAssignmentInitializer
                        : null;
                    if (
                        ts.isBinaryExpression(target) &&
                    ...
                    const step = ts.isObjectLiteralExpression(pattern)
                        ? names?.length === 1
                            ? { kind: "property", names }
                            : null
                        : { kind: "array", index };
                    ...
                    const nestedReceivers = receiversThroughBindingDefault(
                        projected,
                        defaultValue,
                        "nested implicit accessor assignment receivers",
                    );
                    recordAssignmentPatternActivations(target, nestedReceivers, activation);
                }
            }
            for (const sourceFile of program.getSourceFiles()) {
                if (sourceFile.isDeclarationFile) continue;
                function visit(node) {
                    if (ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) {
                        ...
                        const disabledBindingDefault = [...bindingDefaultMayRun].some(
                            ([initializer, mayRun]) => !mayRun && isWithin(node, initializer),
                        );
                        if (!isWrite && !disabledBindingDefault && names?.length) {
                            const directSymbol = ts.isPropertyAccessExpression(node)
                                ...
                                : names.length === 1
                                  ...
                                  : null;
                            const getters = [
                                ...(concreteReachingExpressions(node.expression).length === 0
                                    ? (directSymbol?.declarations ?? []).filter(
                                          (declaration) =>
                                              ts.isGetAccessorDeclaration(declaration) &&
                                              declaration.body,
                                      )
                    ...
                    if (
                        ...
                        recordBindingPatternActivations(node.name, [node.initializer], node);
                    }
                    if (
                        ts.isVariableDeclaration(node) &&
                        (ts.isObjectBindingPattern(node.name) ||
                        ...
                        const iteration = node.parent.parent;
                        for (const receiver of staticArrayEvaluationElements(
                            iteration.expression,
                        ) ?? []) {
                            if (!receiver) continue;
                            recordBindingPatternActivations(node.name, [receiver], iteration);
                        }
                    }
                    if (
                        ts.isParameter(node) &&
                        ...
                        for (const invocation of callsByDeclaration.get(node.parent) ?? []) {
                            ...
                            const receivers = [
                                ...(argument && possibilities.canDefined ? [argument] : []),
                                ...(node.initializer && possibilities.canUndefined
                                    ? [node.initializer]
                                    : []),
                            ];
                            if (receivers.length > 0) {
                                recordBindingPatternActivations(
                                    node.name,
                                    receivers,
                                    invocation.node,
                                );
                    ...
                    if (
                        ...
                        recordAssignmentPatternActivations(node.left, [node.right], node);
                    }
                    ...
                    if (ts.isCallExpression(node)) {
                        if (indexRuntimeGetters) {
                            recordRuntimeGetterDefinition(
                                node.expression,
                                [...node.arguments],
                                node,
                            );
                        ...
                        const adapter = invocationAdapter(node.expression, node.pos);
                        const boundAlternatives =
                            ts.isIdentifier(node.expression) || ts.isCallExpression(node.expression)
                                ? boundFunctionAlternatives(node.expression, node.pos)
                                : [];
                        const normalized =
                            adapter ||
                            boundAlternatives.some((alternative) => alternative.kind !== "plain")
                                ? normalizeEffectiveCalls(
                                      node.expression,
                                      [...node.arguments],
                                      node.pos,
                        ...
                        for (const call of normalized) {
                            ...
                            if (
                                ...
                                const names = literalPropertyNames(
                                    checker.getTypeAtLocation(call.arguments[1]),
                                );
                                if (names?.length) {
                                    recordAccessorActivation(
                                        localGetterDeclarations(call.arguments[0], names),
                                        node,
                                        call.arguments[2] ?? call.arguments[0],
                                    );
                                }
                            }
                        }
        ...
        const activations = [
            ...(callsByDeclaration.get(owner) ?? []).flatMap((invocation) =>
                parameterSubstitutionAlternatives(owner, invocation).map((substitutions) => ({
                    node: invocation.node,
                    substitutions,
                })),
            ),
            ...(accessorReadsByDeclaration.get(owner) ?? []).flatMap((node) => {
                const receivers = accessorReceiverByActivation.get(node) ?? [null];
                return receivers.map((receiver) => {
                    const substitutions = new Map();
                    if (ts.isGetAccessorDeclaration(owner) && receiver) {
                        substitutions.set(THIS_SUBSTITUTION, receiver);
                    }
    ...
    function writeNeedsBoundaryProjection(write, use) {
        ...
        function isAsyncIifeOwner(owner) {
            if (
            ...
            if (ts.isCallExpression(value.parent) && value.parent.expression === value) return true;
            if (
                ts.isVariableDeclaration(value.parent) &&
                value.parent.initializer === value &&
                ts.isIdentifier(value.parent.name) &&
                enclosingFunction(value.parent)
            ) {
                const invocations = callsByDeclaration.get(owner) ?? [];
                return invocations.length === 1;
            }
            let container = value.parent;
            while (
                ts.isConditionalExpression(container) ||
                (ts.isBinaryExpression(container) &&
                    [
                        ts.SyntaxKind.BarBarToken,
                        ts.SyntaxKind.AmpersandAmpersandToken,
                        ts.SyntaxKind.QuestionQuestionToken,
                    ].includes(container.operatorToken.kind))
            ) {
                value = container;
            ...
            if (
                ...
                container.initializer === value &&
                ts.isIdentifier(container.name) &&
                enclosingFunction(container)
            ) {
                return (callsByDeclaration.get(owner) ?? []).length === 1;
            }
            return false;
        }
        return Boolean(
            ...
            (callsByDeclaration.get(writeOwner) ?? []).some(
                (invocation) =>
                    invocation.requiresAwaitedCompletion || isAsyncIifeOwner(writeOwner),
            ),
        );
    }
    ...
    function traceBindingElement(binding, context, depth, seen) {
        ...
        function literalSelection(source, key) {
            if (isRestArgumentsSubstitution(source) && typeof key === "number") {
                return {
                    known: true,
                    value: source[REST_ARGUMENTS_SUBSTITUTION][key] ?? null,
                };
            }
        ...
        function traceStep(source, stepIndex, propertyDepth, propertySeen, stepContext = context) {
            ...
            const [restAccess, ...remainingAccessPath] = stepContext.accessPath ?? [];
            if (
                step.rest &&
                ts.isObjectBindingPattern(step.binding.parent) &&
                restAccess?.kind === "property" &&
                restAccess.names.length === 1 &&
                !step.excluded.includes(restAccess.names[0])
            ) {
                const alternatives = memberValueAlternatives(
                    source,
                    restAccess.names[0],
                    stepContext.bindingUseExpression ?? source,
                    stepContext.bindingUsePosition ?? source.pos,
                    null,
                    true,
                );
                const values = alternatives
                    ...
                    .filter(Boolean);
                for (const value of values) {
                    traceStep(value, stepIndex + 1, propertyDepth + 1, propertySeen, {
                        ...stepContext,
            ...
            if (!selected.known && !step.rest && step.key != null) {
                const alternatives = memberValueAlternatives(
                    source,
                    String(step.key),
                    stepContext.bindingUseExpression ?? source,
                ...
                const values = alternatives
                    ...
                    .filter(Boolean);
                for (const value of values) {
                    traceStep(value, stepIndex + 1, propertyDepth + 1, propertySeen, stepContext);
                }
                if (values.length > 0) return;
            }
            if (ts.isIdentifier(source)) {
                const symbol = unalias(checker, checker.getSymbolAtLocation(source));
                const substituted = stepContext.substitutions?.get(symbol);
                if (isRestArgumentsSubstitution(substituted)) {
                    traceStep(substituted, stepIndex, propertyDepth + 1, propertySeen, stepContext);
                    return;
                }
                const restParameter = (symbol?.declarations ?? []).find(
                    (candidate) => ts.isParameter(candidate) && candidate.dotDotDotToken,
                );
                if (restParameter && ts.isFunctionLike(restParameter.parent)) {
                    const parameterIndex = restParameter.parent.parameters.indexOf(restParameter);
                    let tracedRest = false;
                    for (const invocation of callsByDeclaration.get(restParameter.parent) ?? []) {
                        for (const values of restArgumentLists(
                            invocation.arguments,
                            parameterIndex,
                            invocation.node.pos,
                        )) {
                            traceStep(
                                restArgumentsSubstitution(values),
                                stepIndex,
                                propertyDepth + 1,
                                propertySeen,
                                {
                                    ...stepContext,
                                    bindingUsePosition: invocation.node.pos,
                                    bindingUseExpression: invocation.node,
                                },
                            );
                            tracedRest = true;
                        }
                    }
                    if (tracedRest) return;
                }
        ...
        for (const invocation of invocations) {
            if (declaration.dotDotDotToken) {
                for (const values of restArgumentLists(
                    ...
                    traceStep(restArgumentsSubstitution(values), 0, depth + 1, seen, {
                        ...context,
                        bindingUsePosition: invocation.node.pos,
                        bindingUseExpression: invocation.node,
                    });
            ...
            const argument = invocation.arguments[parameterIndex];
            if (argument && !isDefinitelyUndefined(argument)) {
                    ...
                    if (declaration.initializer) {
                        ...
                        const defaultMayRun =
                            ...
                            calls.some((call) => {
                                ...
                                return argument
                                    ? undefinedPossibilities(argument).canUndefined
                                    : true;
                            });
                        if (defaultMayRun) {
            ...
            if (projectionRoot.kind === ts.SyntaxKind.ThisKeyword) {
                ...
                for (const projection of thisProjections) {
                    ...
                    for (const path of rootProjectionPaths.length > 0
                        ? rootProjectionPaths
                        : [[]]) {
                        function traceReceiverPath(receiver, pathIndex) {
                            if (pathIndex > MAX_TRACE_DEPTH) {
                                recordTraceDepthFailure(context);
                                return;
                            }
                            ...
                            const names =
                                step.kind === "property"
                                    ...
                                    : step.kind === "array"
                                      ...
                                      : null;
                            for (const name of names ?? []) {
                                let alternatives = memberValueAlternatives(
                                    receiver,
                                    name,
                                    expression,
                                    projection.node.pos,
                                ...
                                if (alternatives.length === 0) {
                                    alternatives = bounded(
                                        reachingExpressionValues(
                                            ...
                                            projectedExpressions(value.expression, [
                                                { kind: "property", names: [name] },
                                            ]).map((projected) => ({ value: projected })),
                                        ),
                                        "projected this receiver alternatives",
                                    );
                                }
                                for (const alternative of alternatives) {
                                    if (alternative?.value) {
                                        traceReceiverPath(alternative.value, pathIndex + 1);
                                    }
                                }
                            }
                        }
                        traceReceiverPath(projection.receiver, 0);
                    }
                }
                return;
            }
