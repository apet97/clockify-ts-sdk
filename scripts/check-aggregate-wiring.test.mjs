...
try {
    ...
    // 2. Remove `support-bundle` ONLY from the governance-audit aggregate prerequisite line.
    ...
    assert.notEqual(
        runScript(),
        0,
        "checker must fail when support-bundle is removed from governance-contracts",
    );
} finally {
    writeFileSync(makefilePath, original);
}

console.log("ok - aggregate wiring self-check is scoped to governance-audit");
