...
const failures = [];
const contract = readJson("docs/ci-contract.json", "contract") ?? {};
...
function validateEntryCollection(label, entries) {
    ...
    for (const [index, entry] of entries.entries()) validateEntryShape(`${label}[${index}]`, entry);
    assertUnique(
        `${label}.path`,
...
function validateContractShape() {
    ...
    for (const invariant of [
        ...
        "valid-purpose",
        "safe-ci-evidence-paths",
        "typed-policy-document-contract",
        "typed-workflow-entries",
        "typed-supporting-doc-entries",
    ...
    validateEntryCollection("workflows", contract.workflows);
    validateEntryCollection("supportingDocs", contract.supportingDocs);
...
if (failures.length > 0) {
    console.error("CI contract shape failed");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
}

checkEntry(contract.policyDocument);
for (const entry of contract.workflows ?? []) checkEntry(entry);
