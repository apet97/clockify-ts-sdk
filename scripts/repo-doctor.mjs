...
export async function buildReport() {
    ...
    checks.push(
        check(
            ...
            (await exists("package.json")) ? "fail" : "pass",
            "Root package.json must not exist because this repo is intentionally not an npm workspace",
        ),
    );
    checks.push(
        check(
            "root.no-package-lock",
            (await exists("package-lock.json")) ? "fail" : "pass",
            "Root package-lock.json must not exist because package installs are package-local",
        ),
    );

    for (const pkg of packages) checks.push(...(await packageChecks(pkg)));

    const fernConfig = (await exists("spec/fern/fern.config.json"))
        ? await readText("spec/fern/fern.config.json")
        : "";
    const fernGenerators = (await exists("spec/fern/generators.yml"))
        ? await readText("spec/fern/generators.yml")
        : "";

    checks.push(
        check(
            "fern.cli-pin",
            fernConfig.includes('"version": "5.37.9"') ? "pass" : "fail",
            "Fern CLI pin should remain 5.37.9",
        ),
    );
    checks.push(
        check(
            "fern.generator-pin",
            fernGenerators.includes("fernapi/fern-typescript-node-sdk") &&
                fernGenerators.includes("version: 3.71.2")
                ? "pass"
                : "fail",
            "Fern TypeScript generator should remain fernapi/fern-typescript-node-sdk 3.71.2",
        ),
    );
    checks.push(
        check(
            "fern.output-path",
            fernGenerators.includes("path: ../../output/ts-sdk") ? "pass" : "fail",
            "Fern output path should remain ../../output/ts-sdk",
        ),
    );
    ...
    checks.push(
        check(
            ...
            "output/ts-sdk is generated by Fern and may be absent before regeneration",
        ),
    );
    ...
    return {
        ...
        repo: {
            ...
            packageModel: "package-local",
            rootPackageJsonAllowed: false,
            network: "none",
            commandsExecuted: [],
        },
        environmentShape: {
        ...
        next:
            summary.fail > 0
                ...
                : summary.warn > 0
                  ? ["Run package setup or GOCLMCP/Fern generation only if that surface is needed."]
