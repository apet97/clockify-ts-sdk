...
export interface OperationSpec {
    ...
    contentType?: string;
    multipart?: boolean;
    responseType?: "json" | "binary" | "mixed" | "void";
}
    ...
    try {
        return await executeRequest(
            ...
            async (response) => {
                ...
                // Error responses use their own media type. A binary success
                // declaration must not hide a JSON error envelope inside a
                // BinaryResponse wrapper.
                const data = await parseBody(response, response.ok ? operation.responseType : "json");
...
async function parseBody(response: Response, responseType: OperationSpec["responseType"]): Promise<unknown> {
    if (responseType === "binary" || responseType === "mixed") {
        return getBinaryResponse(response) satisfies BinaryResponse;
    }
...
function responseTypeFromOperation(operation) {
    ...
    const type = typeFromSchema(operation.response.schema, activeModel);
    const jsonType = operation.response.allowsEmptyBody ? `${type} | undefined` : type;
    return operation.response.type === "mixed" ? `core.BinaryResponse<${jsonType}>` : jsonType;
}
