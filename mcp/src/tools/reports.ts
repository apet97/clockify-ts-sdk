...
export function registerReportsTools(server: McpServer, ctx: Context): void {
    defineTool(
        ...
        "clockify_reports_summary",
        {
            ...
            description: "Run a summary report over a date range, grouped per summaryFilter.groups (e.g. PROJECT, CLIENT).",
            inputSchema: {
                ...reportCore,
                summaryFilter: z.record(z.unknown()).describe('e.g. { "groups": ["PROJECT", "TASK"] }'),
            },
    ...
    defineTool(
        ...
        "clockify_reports_detailed",
        {
            ...
            description: "Run a detailed report listing individual time entries over a date range, paginated via detailedFilter.",
            inputSchema: {
                ...reportCore,
                detailedFilter: z.record(z.unknown()).describe('e.g. { "page": 1, "pageSize": 50 }'),
            },
            annotations: { readOnlyHint: true, idempotentHint: true },
        },
        async (args) => {
    ...
    defineTool(
        ...
        {
            title: "Weekly report",
            description: "Run a weekly report aggregating tracked time per week over a date range, grouped per weeklyFilter.",
            inputSchema: {
                ...reportCore,
                weeklyFilter: z.record(z.unknown()).describe('e.g. { "group": "USER", "subgroup": "PROJECT" }'),
            },
            annotations: { readOnlyHint: true, idempotentHint: true },
        },
        async (args) => {
    ...
    defineTool(
        ...
        "clockify_reports_attendance",
        {
            ...
            description: "Run an attendance report of clock-in/out and break activity over a date range, scoped by attendanceFilter.",
            inputSchema: {
                ...reportCore,
                attendanceFilter: z.record(z.unknown()).describe('e.g. { "users": { "ids": ["user-id"], "contains": "CONTAINS" } }'),
            },
            annotations: { readOnlyHint: true, idempotentHint: true },
        },
        async (args) => {
