...
export function registerTimeOffTools(server: McpServer, ctx: Context): void {
    ...
    // ---- balance assignments ----
    //
    // A balance assignment is the per-(user, policy) balance record.
    // Live-verified 2026-08-05 against the sandbox: create is additive (it
    // adds to an existing assignment and creates one only when absent),
    // update applies a delta, and delete requires a note.
    ...
    defineGuardedTool(
        ...
        "clockify_time_off_balance_assignments_create",
        {
            ...
            inputSchema: {
                ...
                balance: zNumberLike(z.number().finite()).describe(
    ...
    defineGuardedTool(
        ...
        "clockify_time_off_balance_assignments_update",
        {
            ...
            inputSchema: {
                ...
                policyId: z.string().min(1).describe("Policy id (24-hex) or exact policy name."),
                userId: z.string().min(1).describe("Workspace user id, exact name, email, or `me`."),
                balanceChange: zNumberLike(z.number().finite()).describe(
