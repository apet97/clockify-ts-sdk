...
| Install / run | Version `6.0.1`: local stdio and optional one-click `.mcpb`, plus stateless HTTP and separate admin binaries in the npm package or source-built container; remote components are deliberately absent from the bundle | Clone + build | Hosted sign-up / OAuth |
