...
| `.github/dependabot.yml`                            | edit-only, no gates (GitHub validates on next dependabot poll); commits with `chore(deps)` / `chore(dev-deps)` / `chore(ci)` prefixes per the file's commit-message config. |
...
│   │                            gen-resource-docs.ts so the markdown reference
│   │                            stays in sync with the synced SDK
...
│   └── gen-resource-docs.ts  ← parses src/api/resources/*/client/{Client.ts,
│                                requests/*.ts} → emits docs/resources/<name>.md
│                                (committed, one per of the 31 resources + README
│                                index covering all 190 methods)
...
│                                via the repo URL.
├── docs/                     ← markdown reference (Phase 3.4) — 31 per-resource
│                                .md files + index README.md. Auto-generated by
│                                scripts/gen-resource-docs.ts; committed.
