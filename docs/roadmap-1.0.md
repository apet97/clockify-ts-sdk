...
| 17. CLI mutation target | 16 | implemented; approvals pending | exact CLI scope is `leaf-command.ts`, `resolve-refs.ts`, and `receipt.ts`, with global floor 96 and module floors 95/95/100; final GitHub run `29913220026` passed at floor-bearing head `9dfc3bf` and retained `mutation-reports-cli-1`; 0/2 approvals are recorded | Two independent approvals of the final CLI proof; `docs/roadmap-1.0-receipts/task-17-cli-mutation.md` records calibration/post-test non-authority, final run, and retained artifact | Yes |
