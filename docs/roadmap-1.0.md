...
| 15. Wrapper replacement mutation configuration | 14 | implemented — approvals pending | exact positive-scope/floor equality; `ensure.ts` 94 and `invoice-body.ts` 93 under global 82; GitHub run `29900533134` passed at final head `e65ec4d`; retained `mutation-reports-wrapper-1`; independent approvals 0/2 | GitHub Actions `Mutation`, target `wrapper`; `docs/roadmap-1.0-receipts/task-15-wrapper-replacement-mutation.md` records calibration, ratchet, final run, and retained artifact | Yes — 0/2 approvals recorded |
