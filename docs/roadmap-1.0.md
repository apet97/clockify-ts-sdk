...
| 8. Authenticated-host equality | 7 | evidence_captured (0/2 approvals) | fail-closed host-set equality test (hand-written = generated = emitter = per-op hosts = policy) + config-precedence contract anchor; receipt tracked | `npm test -w clockify-sdk-ts-115 && make config-precedence`; `docs/roadmap-1.0-receipts/task-08-authenticated-host.md` | Yes |
| 9. Shared exact-artifact engine | 8 | evidence_captured (0/2 approvals) | engine prints sha512 digests, adds `--package` release-proof modes + MCP stdio smoke + fail-closed arg tests; receipt tracked | `make pack-smoke`; `docs/roadmap-1.0-receipts/task-09-artifact-engine.md` names all package tarballs and consumer commands | Yes |
| 10. Wrapper release-proof adoption | 9 | evidence_captured (0/2 approvals) | prepublishOnly ends with `--package=wrapper` exact-artifact proof; digest + consumer-install output recorded | `npm run prepublishOnly -w clockify-sdk-ts-115`; `docs/roadmap-1.0-receipts/task-10-wrapper-release-proof.md` has the tarball digest and consumer-install output | Yes |
| 11. CLI release-proof adoption | 10 | evidence_captured (0/2 approvals) | prepublishOnly ends with `--package=cli` exact-artifact proof; digest + packed-bin `--version` output recorded | `npm run prepublishOnly -w @apet97/clockify-cli-115`; `docs/roadmap-1.0-receipts/task-11-cli-release-proof.md` has the tarball digest and bin smoke output | Yes |
| 12. MCP release-proof adoption | 11 | evidence_captured (0/2 approvals) | prepublishOnly ends with `--package=mcp` exact-artifact proof; digest + packed-server stdio initialize/tools-list output recorded | `npm run prepublishOnly -w @apet97/clockify-mcp-115`; `docs/roadmap-1.0-receipts/task-12-mcp-release-proof.md` has the tarball digest and stdio smoke output | Yes |
| 13. Manual governance receipt | 12 | evidence_captured (0/2 approvals) | closure ran solo with make exit 0; receipt names the SDK/CLI/MCP sha512 digests and consumer outputs; `cross-package-release-proof-asymmetry` closed per its gate | `make perfect-full pack-smoke release-readiness`; `docs/roadmap-1.0-receipts/task-13-exact-artifact.md` names SDK, CLI, and MCP digests | Yes |
...
| 22. Webhook delivery diagnosis | 21 | evidence_captured (0/2 approvals) | read-only SDK status/latest-log projection with response-body redaction; deterministic MCP proof is tracked in `docs/roadmap-1.0-receipts/task-22-webhook-diagnosis.md`; closure command exited 0; remaining blocker: two independent approvals; live sandbox read probe not run | `npm test -w @apet97/clockify-mcp-115`; `docs/roadmap-1.0-receipts/task-22-webhook-diagnosis.md` | No — post-baseline workflow scope |
| 23. Workspace-user status administration | 22 | pending | blocked on Task 22's two independent approvals; no Task 23 implementation evidence recorded | `npm test -w @apet97/clockify-mcp-115`; `docs/roadmap-1.0-receipts/task-23-workspace-user.md` | No — post-baseline workflow scope |
...
a non-blocking risk. Task 21 is complete at 2/2 approvals; Task 22 is
`evidence_captured` at 0/2 approvals with exactly two independent approvals
remaining, and Task 23 remains pending on those approvals. Task 1 and the roadmap
remain open.
