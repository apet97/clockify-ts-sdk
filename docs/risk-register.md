...
| `cross-package-release-proof-asymmetry` | `accepted` | SDK/CLI/MCP release proof | The shared exact-artifact engine prints sha512 tarball digests, each prepublishOnly runs its single-package proof, and the retained Task 13 receipt names all three digests and consumer outputs. | Closed 2026-07-22 by `make perfect-full pack-smoke release-readiness` (make exit 0), `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), and `docs/roadmap-1.0-receipts/task-13-exact-artifact.md` naming all three tarball digests and consumer outputs. |
| `remote-mutation-proof-pending` | `accepted` | Mutation score proof | The live-verified aggregate `all` Mutation run at the Task 17 proof SHA covers wrapper, MCP, and CLI with the exact three report paths and every current global/module floor. Task 18 has two independent implementation approvals, and the remote-mutation proof risk remains accepted and non-blocking. | Accepted 2026-07-22 after `make mutation-ci`, the separate live verifier, the Task 18 receipt, and the aggregate GitHub Actions run/artifact verification. A new release decision that needs a fresh download dispatches and verifies a new aggregate run. |
...
| `migration-notes-not-derived-from-surface-diff` | `accepted` | Release process / migration guide | scripts/published-surface-diff/ (P1) computes an exact bump-class-aware surface delta on every release-proof run, but the migration guide is still hand-written and does not cite that delta directly. | Closes the first time a post-P1 major version's migration guide entry cites its published-surface-diff delta directly (e.g. quotes the added/removed symbol list P1 printed for that release) rather than being written independently of it. Until then this stays accepted: P1 exists and blocks unlicensed breaks; wiring its output into migration-note generation is deferred, unstarted work, not a defect. |
