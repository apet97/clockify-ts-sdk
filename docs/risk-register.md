...
| `remote-mutation-proof-pending` | `open` | Mutation score proof | Tasks 14 and 15 have retained, independently approved wrapper authentication and replacement mutation proofs; Task 16 has a retained, independently approved MCP safety mutation proof; and Task 17 has a retained CLI proof at its floor-bearing SHA. Two independent reviewers approved the corrected frozen range. The authoritative aggregate all proof required by Task 18 remains incomplete. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make mutation-ci`, and `docs/roadmap-1.0-receipts/task-18-remote-mutation.md` with GitHub Actions Mutation run URLs plus retained mutation-report artifact names for every approved target. |
