...
| `remote-mutation-proof-pending` | `accepted` | Mutation score proof | The live-verified aggregate `all` Mutation run at the Task 17 proof SHA covers wrapper, MCP, and CLI with the exact three report paths and every current global/module floor. Task 18 awaits two independent implementation approvals, but the remote-mutation proof risk is accepted and non-blocking. | Accepted 2026-07-22 after `make mutation-ci`, the separate live verifier, the Task 18 receipt, and the aggregate GitHub Actions run/artifact verification. A new release decision that needs a fresh download dispatches and verifies a new aggregate run. |
