...
| `remote-mutation-proof-pending` | `open` | Mutation score proof | Tasks 14 and 15 have retained, independently approved wrapper authentication and replacement mutation proofs, and Task 16 has a retained, independently approved MCP safety mutation proof; the CLI target and authoritative aggregate all proof required by Task 18 remain incomplete. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make mutation-ci`, and `docs/roadmap-1.0-receipts/task-18-remote-mutation.md` with GitHub Actions Mutation run URLs plus retained mutation-report artifact names for every approved target. |
