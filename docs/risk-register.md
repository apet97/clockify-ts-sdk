...
| `expense-date-filter-contract` | `open` | Expense query contract | Expense date filtering is not yet wire-proven as one typed cross-surface contract. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `npm test -w @apet97/clockify-cli-115 -- tests/read-commands-expenses.test.ts`, `npm test -w @apet97/clockify-mcp-115 -- tests/expenses.test.ts`, `make operation-parity-drift`, and `make contract-gates`. |
| `expense-update-file-schema` | `open` | Expense update request schema | The generated multipart file field and the supported scalar expense update are not yet reconciled. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make sdk-codegen-test consumer-cast-budget`, `npm test -w @apet97/clockify-cli-115 -- tests/read-commands-expenses.test.ts`, and `make contract-gates`. |
| `operation-parity-generated-reachability` | `open` | Generated operation parity | The 169/156 generated parity map is inferred from OpenAPI names, not a generated-method reachability receipt. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make sdk-codegen sdk-codegen-drift operation-parity-drift contract-gates`, and `docs/roadmap-1.0-receipts/task-05-generated-reachability.md`. |
| `consumer-request-casts` | `open` | CLI and MCP request typing | Request casts remain a temporary escape hatch rather than a zero-cast public boundary. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make consumer-cast-budget contract-gates`, `npm test -w @apet97/clockify-cli-115`, `npm test -w @apet97/clockify-mcp-115`, and `docs/roadmap-1.0-receipts/task-07-zero-cast.md`. |
