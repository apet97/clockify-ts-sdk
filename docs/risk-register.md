...
| `consumer-request-casts` | `accepted` | CLI and MCP request typing | CLI and MCP request construction has zero escape-hatch request casts under a source-aware, canonical-zero ratchet. | Closed 2026-07-19 by `make consumer-cast-budget risk-register contract-gates`, full wrapper public type proof, full CLI/MCP lint/type/test/build proof, and `docs/roadmap-1.0-receipts/task-07-zero-cast.md`. |
