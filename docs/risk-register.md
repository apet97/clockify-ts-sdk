...
| `remote-mutation-proof-pending` | `open` | Mutation score proof | Task 14 has a retained wrapper authentication mutation proof and Task 15 has a retained wrapper replacement mutation proof, but the aggregate approved-target proof required by Task 18 remains incomplete. | `make risk-register` (including `scripts/check-risk-register.readiness.test.mjs`), `make mutation-ci`, and `docs/roadmap-1.0-receipts/task-18-remote-mutation.md` with GitHub Actions Mutation run URLs plus retained mutation-report artifact names for every approved target. |
