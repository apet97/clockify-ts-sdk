...
| `consumer-request-casts` | `open` | CLI and MCP request typing | CLI and MCP request construction currently measures zero escape-hatch request casts, but the corrected symbol-aware ratchet is pending independent reviewer approval. | Pending independent reviewer approval after `make consumer-cast-budget risk-register contract-gates`, full wrapper/CLI/MCP lint/type/test/build proof, package proof, and the corrected `docs/roadmap-1.0-receipts/task-07-zero-cast.md`. |
