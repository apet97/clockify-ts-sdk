...
|---|---|---|---|
| Credential handling | `CLOCKIFY_API_KEY`, `CLOCKIFY_ADDON_TOKEN`, `CLOCKIFY_WORKSPACE_ID`, or `NPM_TOKEN` leaks through docs, examples, errors, or logs. | Keep secrets in environment variables only, use token-shaped scans, and keep error/receipt paths redaction-friendly. | `make secret-hygiene`, `make env-contract`, `make receipts-contract` |
...
| Live proof | Live tests mutate a customer workspace or leave sandbox records behind. | Require explicit live env vars, sacrificial workspace only, timestamped prefixes, paired cleanup, and final cleanup receipts. | `make live-safety`, `make perfect-live`, `make final-proof-receipt-check` |
...
| First-run support handoff | A setup/support issue jumps from local diagnostics to raw logs, env dumps, live Clockify calls, mutation, or release changes. | Start with `node scripts/workflow-plan.mjs --workflow first-run-support`, preserve only safe `safeCommandHints`, and keep the workflow map no-network until an operator deliberately runs mock or sandbox proof. | `make workflow-cookbook`, `make support-bundle`, `make data-handling`, `make security-threat-model` |
| Observability receipts | Request IDs, errors, or spans are missing recovery data or accidentally expose secrets. | Preserve `X-Request-Id`, raw response helpers, stable error codes, OTel hooks, CLI JSON errors, and MCP structured envelopes. | `make receipts-contract`, `make sdk-runtime-contract`, `make cli-contract`, `make mcp-contract` |
