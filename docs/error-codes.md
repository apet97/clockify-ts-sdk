...
| `name_reserved_after_delete` | no | - | no | sdk, cli, mcp | A project, tag, or client with that name already exists. Archived entities keep holding their name, so the conflict is often with something the list call hides by default. (Live probe 2026-08-07: deleting an entity releases its name immediately, so the code's own name is a misnomer kept for compatibility.) | List the entity type including archived rows to find the holder, then reuse it, rename it, or delete it. Waiting does not help — the name is held for as long as the entity exists. |
