...
| `feature_unavailable` | yes | 402 | no | sdk, cli, mcp | The endpoint exists but the workspace plan or feature configuration does not expose it. | Use a supported workspace plan, skip the gated workflow, or fall back to a lower-level read operation. |
...
| `name_reserved_after_delete` | no | - | no | sdk, cli, mcp | A project, tag, or client with that name already exists. Archived entities keep holding their name, and an archived row is easy to miss in a long list even though the list call returns it. (Live probe 2026-08-07: deleting an entity releases its name immediately, so the code's own name is a misnomer kept for compatibility.) | List the entity type and look for an archived holder — the list returns archived and active rows by default (live probe 2026-08-09) — then reuse it, rename it, or delete it. Waiting does not help: the name is held for as long as the entity exists. |
