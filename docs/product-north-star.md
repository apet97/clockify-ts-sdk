...
| `mcp/` / `@apet97/clockify-mcp-115` | Agent-facing Clockify workflows | Workflow-first tools, 163 registered tools with 162 advertised by default, dual-era stdio, authenticated stateless HTTP, one bounded Reports App, rich result envelopes, recovery hints, dry-run confirmation tokens, and clean live sandbox cleanup. |
