...
| Run governed live-evidence campaign | `make live-evidence-campaign` | Rebuilds the SDK offline, runs the credentialed 168-operation campaign (the current live-evidence manifest; seven deprecated documented-only operations remain outside live proof) from an immutable content snapshot, proves final cleanup, and emits hash-bound candidates for separate human approval. |
