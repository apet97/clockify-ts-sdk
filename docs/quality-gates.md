...
| Check mutation CI wiring | `make mutation-ci` | The manual GitHub Mutation workflow stays dispatch-only on exact Node 22.13.0, uses SHA-pinned actions with 14-day report retention, fails on missing reports, and keeps local Stryker concurrency capped. |
