...
| Check mutation CI wiring | `make mutation-ci` | The manual GitHub Mutation workflow stays dispatch-only on exact Node 22.13.0, uses SHA-pinned actions with a complete-history checkout and 14-day report retention, verifies the exact target-aware report set before one target/run-attempt artifact upload, and fails on missing reports or any shallow repository. Isolated Git repositories prove maximum-floor and governed-package/module union retention across every contract-changing first-parent commit, including skipped-workflow decreases, deletion/reintroduction, legitimate additions/raises, and verified root/first-introduction bootstrap. The gate also generates manifest-derived runtime constants before Stryker starts, pins the governed MCP and CLI test inventories, and keeps local Stryker concurrency capped. |
