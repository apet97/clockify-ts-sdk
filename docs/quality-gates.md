...
| Check gate-tier inventory drift | `make gate-tier-inventory-drift` | Fails when the committed gate topology or decision inventory no longer matches the Makefile. |
...
| Run governed live-evidence campaign | `make live-evidence-campaign` | Rebuilds the SDK offline, runs the credentialed 168-operation campaign from an immutable content snapshot, proves final cleanup, and emits hash-bound candidates for separate human approval. |
...
| Check OpenAPI contract lint | `make openapi-lint` | Corrected OpenAPI operation count, operation IDs, tags, responses, SDK naming coverage, pagination, and Last-Page stamps stay within expected bounds. Reached from `product-contracts`, so `contract-gates` and CI run it. It was standalone until 2026-08-06 and its operation-count pin went stale for a whole re-snapshot as a result (163 while the surface was 168); the wiring landed with the campaign run that re-recorded live-evidence currentness, because the `Makefile` is a governed campaign input. |
...
| Check live-evidence currentness | `make live-evidence-currentness` | The imported manifest, sanitized campaign receipt, exact human approval, and every governed campaign input still match their recorded hashes; missing or stale chain-of-custody evidence is release-blocking. |
| Check service routing matrix | `make service-routing-matrix` | Every service/profile URL row is backed by a source pointer, and every corrected-spec operation resolves to exactly one recognized service host. Source-only (ROUTE-001); no runtime routing behavior depends on this yet. |
...
| Check config precedence | `make config-precedence` | SDK option/env fallback, CLI flag/env/rc order, local MCP env startup, remote MCP stored routing with local credential-variable rejection, and base URL override safety stay aligned. |
...
| Check agent handoff | `make agent-handoff` | AGENTS/CLAUDE guidance, the closed plan lifecycle, evidence-only closeout rules, generated-path rules, temporary context, stale-count markers, and the AGENTS.md skills list vs `.claude/skills/` stay aligned. |
...
| Prove remote MCP service | `make mcp-remote-proof` | A digest-pinned ephemeral PostgreSQL instance and local OAuth/Clockify fixtures prove migrations, JWT/opaque auth, stateless HTTP, grant ceilings, stdin-only admin credential set/validate, encryption, confirmation isolation/quotas, rotation, and cleanup. Deliberately outside `perfect-fast`. |
| Prove remote MCP against Clockify | `make mcp-remote-live-proof` | After the normal remote proof is green, use the exact confirmed sacrificial workspace to prove JWT and opaque status calls, five report/App paths, retry-safe DEMO seed, cross-request confirmation cleanup, database teardown, direct fallback cleanup, and zero leftovers. Credentialed and deliberately outside every offline aggregate. |
| Smoke remote OCI image | `make mcp-container-smoke` | Prove the fail-closed build context, inspect the immutable image's user, entrypoint, OCI labels, and locked runtime dependency structure, run both `--help` entrypoints, and remove every proof-owned resource. This structural smoke does not start service dependencies. |
| Prove remote OCI service | `make mcp-container-service-proof` | Independently build and bind one exact image ID, recheck its runtime structure, then run two read-only, non-root replicas with shared digest-pinned PostgreSQL and a synthetic TLS OAuth fixture. Prove alternating JWT/opaque stateless discovery, cross-replica confirmation visibility and wrong-argument burn with zero Clockify dispatches, admitted-request drain during SIGTERM, survivor readiness, deterministic startup-failure precedence, and complete cleanup. |
...
| Check lint config | `make lint-config` | All three ESLint flat configs (wrapper, cli, mcp) stay on `strictTypeChecked` with zero rules downgraded to `warn`, `consistent-type-imports` enabled, and every hand-authored disable outside the `tests/**` blanket exemption matches one of the 6-entry rationale-backed allowlist in `docs/lint-config-contract.json`. |
| Check tsconfig parity | `make tsconfig-parity` | The three packages' tsconfig.json files hold required strictness flags equal, declared per-package diffs (module target/resolution, lib, output-shape flags) stay honest against the real files, and `verbatimModuleSyntax` is never literally enabled anywhere -- the same guarantee is enforced by `lint-config`'s `consistent-type-imports` requirement instead. |
| Check gate reachability | `make gate-reachability` | Every `scripts/check-*.mjs` file is executed by the Makefile, a root `package.json` script, or a workflow, or is a recorded licensed exception with a reason, a who, and a when -- catching a checker script that exists, compiles, and is even well-tested in isolation, but never actually runs as part of any real gate. |
| Check MCP schema parity (W2a) | `make mcp-schema-parity` | `docs/mcp-tool-schemas.json` (every registered MCP tool's real Zod input schema, serialized structurally) and `docs/mcp-tool-manifest.json` stay fresh against the built server, and every `widened-type` coercion-family license in `docs/surface-divergence-licenses.json` (the `zStringList`/`zNumberLike` helpers) stays source-anchored both directions: a new unlicensed coercion helper reds, and a licensed helper with no real call site left in `mcp/src/tools/**` reds as rot. The full per-operation extra/omitted-param diff (W2b) is deliberately not yet implemented -- see the contract's purpose. |
...
| Check published-vs-candidate surface diff (P1) | `make published-surface-diff` | Fetches the last-published npm tarball for the SDK/CLI/MCP (H4: `npm view` -> download -> verify sha512 -> unpack, fails closed on any registry/network/integrity problem), extracts its runtime surface, extracts the local candidate build's surface with the same extractor pointed at a different root (both sides read compiled `dist/`, never a hand-curated doc, for symmetry), and blocks when the observed delta is not allowed at the version step between the two: any delta at patch or unchanged version, a removal at minor, nothing blocked at major. `release-proof` tier only -- dials out to the real npm registry, the same premise `npm audit` already has in `perfect-fast`. |
...
| Check mutation score floors | Mutation workflow (`workflow_dispatch`) — **never run Stryker locally** | Wrapper + MCP + CLI Stryker mutation testing stays at or above pinned covered-mutant score floors and fails closed if a governed module has zero covered mutants. All three packages carry measured floors (wrapper 82, mcp 85, cli 96); the CLI's temporary first-calibration zero-floor fields were removed once measured floors landed, and the contract tests now reject their reintroduction. Measure only via the manual GitHub Mutation workflow: a local run overwrites the `reports/mutation/mutation.json` that `check-mutation-score.mjs` grades, so a stale or partial local report yields a wrong score rather than a missing one. |
| Check mutation CI wiring | `make mutation-ci` | The GitHub Mutation workflow runs weekly (Mondays 05:00 UTC) and on manual dispatch, on exact Node 22.13.0, uses SHA-pinned actions with a complete-history checkout and 14-day report retention, and verifies the exact target-aware report set before one target/run-attempt artifact upload. It retains historical maximum floors and the governed-package/module union. It also validates the remote-proof record and fixture-only verifier behavior offline; only the separate live verifier contacts GitHub and downloads the retained artifact. |
