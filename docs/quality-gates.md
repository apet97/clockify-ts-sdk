...
| Check mutation CI wiring | `make mutation-ci` | The manual GitHub Mutation workflow stays dispatch-only on exact Node 22.13.0, uses SHA-pinned actions with a two-generation checkout and 14-day report retention, fails on missing reports, proves committed/worktree monotonic-floor behavior in isolated Git repositories, generates manifest-derived runtime constants before Stryker starts, pins the governed MCP test inventory, and keeps local Stryker concurrency capped. |
