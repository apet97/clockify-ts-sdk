...
| Check OpenAPI contract lint | `make openapi-lint` | Corrected OpenAPI operation count, operation IDs, tags, responses, SDK naming coverage, pagination, and Last-Page stamps stay within expected bounds. **Standalone: no aggregate reaches it.** Its operation-count pin went stale for a whole re-snapshot because of that (163 while the surface was 168). Run it by hand after any spec re-snapshot. Wiring it into `product-contracts` is a `Makefile` edit, and the `Makefile` is a governed live-evidence campaign input, so the wiring has to ride along with the next campaign run rather than land on its own. |
...
| Check mutation CI wiring | `make mutation-ci` | The manual GitHub Mutation workflow stays dispatch-only on exact Node 22.13.0, uses SHA-pinned actions with a complete-history checkout and 14-day report retention, and verifies the exact target-aware report set before one target/run-attempt artifact upload. It retains historical maximum floors and the governed-package/module union. It also validates the remote-proof record and fixture-only verifier behavior offline; only the separate live verifier contacts GitHub and downloads the retained artifact. |
