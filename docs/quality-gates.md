...
| Check aggregate execution contracts | `make aggregate-gates` | `perfect-fast`, `perfect-full`, and `contract-gates` retain exact-once Make/verify execution, fast/full performance-last ordering, standalone full/release proof ownership, and no transitively reachable local mutation command. |
...
| Check mutation score floors | `make mutation` | Opt-in local wrapper + MCP + CLI Stryker mutation testing stays at or above pinned covered-mutant score floors and fails closed if a governed module has zero covered mutants. CLI's first remote calibration is the sole explicit temporary zero-floor exception. Prefer the manual GitHub Mutation workflow for routine proof. |
