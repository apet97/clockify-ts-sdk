...
node scripts/plan.mjs acceptance --scenario remote-mcp-reports-app
...
| First-run diagnostics and support handoff | `clockifyDiagnostics()` and quickstart receipt template | `clk115 doctor --json`, `clk115 status --json` | `clockify://mcp/doctor` | `make diagnostics`, `make support-bundle`, `make issue-intake`, `safeCommandHints`, and `readinessContext`. |
...
| OpenAPI truth and generated core | 168 receipt-derived generated SDK methods behind durable wrapper seams (149 explicit + 19 governed operationId-derived) | CLI uses wrapper semantics instead of inventing API truth | MCP uses wrapper semantics and parity metadata | GOCLMCP drift gates, local codegen drift and local codegen receipt, all-operation dispositions, operation coverage, generator comparison, and generated-edit guard. |
| Remote MCP and Reports App | The service builds a per-principal `clockify-sdk-ts-115` client through `createContext(ClockifyContextConfig)`; this is not a direct SDK consumer path. | `clockify115-mcp-admin db migrate`, principal provisioning, and stdin-only `credential set` | `clockify115-mcp-http`, stateless `POST /mcp`, the five report tools, and `ui://clockify115/reports-dashboard` | `make mcp-gates`, `make mcpb-smoke`, `make pack-smoke`, `make mcp-remote-proof`, `make mcp-container-service-proof`, then authorized `make mcp-remote-live-proof` with proof-owned state removed and zero Clockify leftovers. |
| Package-consumer install smoke | packed SDK tarball imports ESM/CJS and subpaths | packed CLI exposes `clockify115` and `clk115` | packed MCP exposes `clockify115-mcp`, `clockify115-mcp-http`, and `clockify115-mcp-admin`; pack smoke imports the server/client/HTTP exports, runs HTTP/admin help, and completes a real packed stdio exchange | `make pack-smoke`, package contract, runtime support, dependency boundary, and supply-chain contract. |
...
3. Quickstart/support proof: `make diagnostics`, `make support-bundle`, and
   `make issue-intake` prove the first-run support
