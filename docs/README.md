...
| Mutation CI wiring | `make mutation-ci` | Check the manual GitHub Mutation workflow stays dispatch-only on exact Node 24.18.0, uses SHA-pinned actions with 14-day report retention, fails on missing reports, and keeps local concurrency capped. Package CI separately proves Node 22.13. |
