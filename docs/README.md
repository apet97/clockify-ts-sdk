...
| Mutation CI wiring | `make mutation-ci` | Check the manual GitHub Mutation workflow stays dispatch-only on exact Node 22.13.0, uses SHA-pinned actions with 14-day report retention, fails on missing reports, generates runtime version constants before Stryker, and keeps local concurrency capped. |
