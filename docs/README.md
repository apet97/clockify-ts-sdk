...
| [`aggregate-gates-contract.json`](./aggregate-gates-contract.json) | edit intentionally | Exact aggregate Make/verify execution sequences, one-execution counts, standalone full/release proof ownership, performance-last ordering, and transitive no-local-mutation bounds. |
| [`aggregate-gates-goclmcp.Makefile`](./aggregate-gates-goclmcp.Makefile) | source-derived fallback | Exact relevant GOCLMCP Make target subset used in single-repository CI and checked target-by-target against the live sibling when present. |
