...
or any other hosted SDK generator platform. The repo-owned local TypeScript
generator is the default emitter, not a hosted product architecture. The product
is the corrected OpenAPI truth, the package contracts, the hand-written wrapper
...
## Repo-owned local generator chain
...
## Local-generator proof checklist

Before changing the local generator or evaluating any replacement platform,
...
repo-owned generator path pinned and replaceable, or evaluate another local
generator in a branch with the local-generator proof checklist above.
