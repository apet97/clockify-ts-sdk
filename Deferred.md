...
`clockify-ts-sdk` at SDK HEAD **`8a29699`** (last code/spec-bearing commit
`4323b1b`; `8a29699` touches this file only), `../GOCLMCP` at **v0.4.5**
...
documented), **92 public SDK names**, 27 subpaths, 58 CLI commands, 134 MCP
tools, `allowedRequestCastBudget: 0` (zero unannotated `as never`). Ground all
"current state" claims on SDK HEAD `8a29699` / GOCLMCP v0.4.5.
...
Each item below carries its own **Done when:** acceptance line, a per-step
**Verify:**, and a **Perfect end state** block describing exactly what the
code/tests/docs look like once the item is fully landed (the target a fresh
session is aiming at). Treat the Done-when as the gate — do not declare an item
...
**Perfect end state.** `grep -rn "as never" cli/src mcp/src | grep -vE "result.ts|output-schema.ts"` returns **exactly 22 cast-operator lines** (24 today − 2 clean drops at `entries.ts:217` + `users.ts:237`), each on/above a sharpened KEEP comment that names *why* it stays (Bucket-C: the missing param/response + a `TODO(GOCLMCP)`; WONTFIX-narrow: "runtime narrow of an `unknown` response"). The 7 A-wireBody sites (`invoices.ts:131/207`, `timeOff.ts:440/542/573`, `expenses.ts:189/238`) carry `wireBody<ClockifyApi.…Body>()` instead of a bare `as never` — still counted by the budget grep but now self-documenting and type-named. Zero remaining *generic* `// KEEP as never: generated list/search/view request or response envelope does not match this wire shape.` comments on Bucket-C sites (each rewritten to name its gap). `make consumer-cast-budget` still prints `0 unannotated as never, budget 0`; both `type-check`s and both suites green. No spec/codegen/runtime change — purely a clearer consumer surface with 2 fewer opaque casts.

### Ground truth (re-validated at SDK HEAD `8a29699`, GOCLMCP v0.4.5)
...
- `grep -rn "as never" cli/src mcp/src | grep -v result.ts` returns **51 lines** (27 KEEP comments + **24 actual cast operators** + the path-exempt hits filtered out). The thing to reduce is the **24 cast-operator sites**; classify each below. (The older "24 KEEP'd sites" via that grep was a miscount of the 51 grep total — the 24 is the cast-operator subset.)
...
grep -rn "as never" cli/src mcp/src | grep -vE "result.ts|output-schema.ts" | grep -v "// KEEP" | wc -l   # => 24 (cast operators)
```
...
| Response narrows: `cli/src/commands/webhooks.ts:29`, `cli/src/commands/expenses.ts:71`, `cli/src/commands/timeoff.ts:59`, `mcp/src/tools/timeOff.ts:149`, `mcp/src/tools/workflows/review.ts:33`, `mcp/src/tools/workflows/resolve.ts:370`, `:111` | response type `unknown`/wrong | **WONTFIX** | the `as never` is on a list/search *request*, and the trailing `as {…}` narrows an `unknown` response; even after a spec response-schema fix a runtime `as` narrow is reasonable. Leave KEEP |
...
- `mcp/src/tools/workflows/resolve.ts:565` FIND the same generic comment REPLACE `// KEEP as never: ListExpenseCategoriesRequest is {workspaceId} only; live accepts page/page-size. TODO(GOCLMCP): add pagination params to listExpenseCategories.`
...
**Bucket-C perfect end state (upstream, for reference).** After the GOCLMCP PR + re-snapshot + `make sdk-codegen`, the 3 C-spec sites (`invoices.ts:43/276/366`, `resolve.ts:566`, `timeOff.ts:267`) bind cleanly with no `as never` and no KEEP comment; `make generator-comparison` shows the new params/response in the regenerated TS methods. The 6 WONTFIX response-narrow sites keep their `as never` + KEEP by design (a runtime `as {…}` narrow of an `unknown` response is reasonable even with a response schema). Not in scope here.
...
**Perfect end state.** Step 0 is merged in GOCLMCP: `grep -n 'go test -tags=livee2e.* -v ' Makefile` returns exactly 2 lines (148, 154), `logFindingsRow` appends to `$CLOCKIFY_LIVE_FINDINGS_OUT`, and `docs/live-tests.md` has the "Harvesting promotable findings rows" block. After the read-tier campaign (Batches 1+2): `grep -c 'x-clockify-live-status: live-success' docs/openapi/clockify-openapi.yaml` reads **≈76** (up from 46), `probe-documented` drops by the same delta, `documented` stays 14, total stays 184; every promoted op's `findings/<domain>.md` row shows a real 2xx (no `TODO-live` rows for promoted ops), `make openapi-drift`/`make perfect-local` green in GOCLMCP. In the SDK: `spec/corrected/clockify.corrected.openapi.yaml` matches the refreshed canonical (diff is `x-clockify-live-status` lines only unless a promotion surfaced an unknown field, in which case the regenerated TS model also changed and `make generator-comparison` flagged it), `make sdk-codegen-drift` acked, `make perfect-fast` green. The structural floor (14 documented + permission-gated + workspace-state-limited + PHANTOM_PATHS) is untouched and correctly NOT live-success — the campaign does not chase those.
...
**Perfect end state (Step 0).** Both live `go test` invocations carry `-v` AND `logFindingsRow` appends to `$CLOCKIFY_LIVE_FINDINGS_OUT` when set, so a green non-`-v`-regression run can never silently swallow rows again. `docs/live-tests.md` documents the harvest grep. A green `make live-contract-local` against a real key prints `≥1` `| GET | … | 200 | … |` row to stdout and writes the same rows to the findings-out file.
...
**Perfect end state.** After B1+B4 + a Stryker re-run: the 8 `dates.ts` StringLiteral survivors (L22 ×3 tue/thu/sat, L33-37 ×5 aug-dec) and the 2 `webhook-url.ts` L189 survivors (`EqualityOperator first < 0xfebf`, `ConditionalExpression→true`) all read `Killed` in `wrapper/reports/mutation/mutation.json`. `wrapper/tests/dates.test.ts` has the two new `it` blocks (every-month-name, every-weekday-name) and `wrapper/tests/webhook-url.test.ts` asserts `[fec0::1]` is accepted. `docs/mutation-score-contract.json` floors are raised ONLY to the measured post-run integers (`wrapper/dates.ts` from 84 to its new floor ~88; `wrapper/webhook-url.ts` 80→81 only if it actually moved). `make mutation` green at the raised floors. Every WONTFIX item (B3 quarter-rollover, B5 confirmation-TTL, A1-A5) remains untouched with its rationale intact — no new tests chasing them. Net: 10 fewer surviving mutants, 0 brittle implementation-accident assertions.
...
Repo HEADs this was re-validated against: clockify-ts-sdk `8a29699`, GOCLMCP `v0.4.5` (`6f3cd2c`).
...
**Perfect end state.** `grep -q "TODO-live" GOCLMCP/docs/openapi/sources/clockify-api-probe-lab/findings/projects.md` returns non-zero (no TODO-live left in that file — all five sibling rows clients/projects/tags/tasks/time-entries read `200`). `clockify-openapi.yaml:5112` still reads `x-clockify-live-status: live-success` and `git diff --quiet docs/openapi/clockify-openapi.yaml` exits 0 after `make gen-openapi` (the row is cosmetic; the stamp already round-trips). `make openapi-drift`, `go test ./tests -run TestGeneratedOpenAPIIgnoresPendingLiveFindingRows`, and SDK-side `make goclmcp-drift` all green. No SDK change, no live key.
...
**Perfect end state.** `grep -h '"vitest"' wrapper/package.json cli/package.json mcp/package.json | sort -u` returns one line `"vitest": "^4.1.4"`; the same for `@vitest/coverage-v8` (`^4.1.9`). The root `package-lock.json` resolves a single hoisted vitest 4.x tree (no nested duplicate). Both `cli` and `mcp` suites + `--coverage` runs pass under v4's v8 reporter, with any reporter-shifted floors re-pinned in lockstep across each package `vitest.config.ts` AND `docs/coverage-contract.json` (the only sanctioned floor edit). `make perfect-fast` + `make changelog-drift` green. The `cli`/`mcp` package changelogs note the bump. No source/behavior change — a pure dev-dependency + threshold-calibration cleanup.
...
**Perfect end state.** `spec/evidence/discrepancies.md`'s `time-off.change-status.union-and-note` entry header no longer reads `PARTIAL` (it reads `COMPENSATED`/`RESOLVED` with the recorded probe outcome). The `as never` at `mcp/src/tools/timeOff.ts:267` is gone — either `note` is required in the tool's Zod input (clean bind) or it stays conditional with the cast dropped — so this site leaves Item 1's WONTFIX list. A new probe under `probes/` records the live missing-`note` 4xx-vs-2xx. `make mcp-write-safety` + `npm test -w @clockify115/mcp-server` green. Decrements Item 1's C-spec count by one.

**Where (verified anchor):** `spec/evidence/discrepancies.md:2290` (`### time-off.change-status.union-and-note — PARTIAL 2026-06-18`; the `note`/`as never` detail is at line 2303). The generated `ChangeTimeOffRequestStatus` marks `note` REQUIRED, but `clockify_time_off_requests_update_status` sets it only when present, with one `as never` (`timeOff.ts:267`) masking the mismatch. The status-union half is already fixed (`z.enum(["APPROVED","REJECTED"])`, test `mcp/tests/sweep-fixes.test.ts`); only the note branch is left.
...
**Perfect end state.** No code change ships unless upstream flips the route. The parity override stays the sole carrier; `make operation-parity` stays green. If a future fake-id probe returns 405 (route exists) instead of 404, the op is added to the GOCLMCP generator FIRST, then re-snapshot — never bolted on in-repo.
...
`spec/evidence/discrepancies.md:2252` (`### compose.work-package.ensure-repoint-wontfix — WONTFIX 2026-06-18`). Re-pointing `createWorkPackage` onto `Workspace.ensure*` is a net regression: it drops server-side name/page-size/clients filters, cannot express `upsert:false` always-create, and cannot carry the per-step `undo` compensations `runComposition` needs (`wrapper/compose.ts:39-40`, `:83-92` — undos run in reverse on failure so there are no orphans). Listed only for completeness.
**Perfect end state.** `wrapper/compose.ts` keeps its direct-create + reverse-undo path unchanged; the discrepancy entry stays `WONTFIX` with the regression rationale. Nothing to do — reopening is explicitly out of scope. Effort: none.

### 4f. wrapper `noImplicitOverride` / `exactOptionalPropertyTypes` — GENERATOR-CHANGE (lives in GOCLMCP). SKIP in-repo.

`spec/evidence/discrepancies.md:2313` (`### strictness.wrapper-eopt-noimplicitoverride-blocked — DOCUMENTED 2026-06-18`). wrapper cannot enable these flags because the errors are in generated `wrapper/src/**` (no-edit hard-stop): `noImplicitOverride` → TS4114 at `src/errors/ClockifyApiError.ts`; `exactOptionalPropertyTypes` → 10 errors across `src/errors/*` and `src/core/request.ts`. Flags stay OFF on wrapper, ON in cli+mcp (`cli/tsconfig.json`, `mcp/tsconfig.json` both set `noImplicitOverride`); rationale pinned in `wrapper/tsconfig.json` `_blockedStrictnessFlags`; the hand-written surface is EOPT-clean and enforced by `scripts/check-consumer-cast-budget.mjs`.
**Perfect end state (upstream).** The GOCLMCP generator emits `override` keywords on overriding members + EOPT-clean optionals, so after re-snapshot + `make sdk-codegen` the wrapper's generated `src/errors/*` + `src/core/request.ts` compile under both flags; `wrapper/tsconfig.json` then enables `noImplicitOverride` + `exactOptionalPropertyTypes` and drops the `_blockedStrictnessFlags` rationale, making all three packages flag-uniform. Until that upstream PR lands, the flags stay OFF on wrapper by design. Effort: M, in GOCLMCP. Keep deferred-to-upstream.
...
2. **Item 1 reducible subset (Buckets A+B)** — cleanest real win: no live key, no codegen change, behavior-neutral, covered by existing tests; removes the 2 clean-drop casts and converts the rest to self-documenting `wireBody<T>()`.
...
**Final/perfect-state of the whole file** (the target a fresh session converges on, if every non-WONTFIX item lands): 4a's findings row reads `200`; Item 1 has 2 fewer cast operators (22) with every remaining `as never` either a typed `wireBody<T>()` or a Bucket-C/WONTFIX KEEP that names its gap; Item 3 shows 10 fewer surviving mutants with floors ratcheted to measured integers; Item 2 (key-permitting) sits at ≈76/184 live-success with the read tier covered; vitest is uniformly v4; 4c is COMPENSATED. The residual is exactly the accurately-recorded WONTFIX / upstream-generator / key-gated diminishing-returns set — that residual IS the perfect state, not a defect.
