...
- **Coordinated package truth:** the SDK is `0.15.1`, the CLI is `0.6.0`, and the
  TypeScript MCP is `0.9.0`. `version-consistency` reconciles all three package
...
- **Current surface:** 153 MCP tools (22 workflow/orientation plus 131
...
| `cli/` | `@apet97/clockify-cli-115` | v0.6.0 CLI; bins `clockify115` and `clk115`; command metadata is generated into the product surface; `--output table\|json\|ndjson`/`--compact`/`--select` controls |
| `mcp/` | `@apet97/clockify-mcp-115` | v0.9.0 stdio MCP; bin `clockify115-mcp`; tool/resource counts are generated into the product surface |
