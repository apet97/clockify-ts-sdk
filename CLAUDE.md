...
- **Coordinated package truth:** the SDK is `0.15.1`, the CLI is `0.5.1`, and the
  TypeScript MCP is `0.8.1`. `version-consistency` reconciles all three package
...
- **Current surface:** 146 MCP tools (22 workflow/orientation plus 124
...
|---|---|---|
| `wrapper/` | `clockify-sdk-ts-115` | v0.15.1 SDK; dual ESM/CJS; public names and subpaths governed by `docs/sdk-public-api.json` |
| `cli/` | `@apet97/clockify-cli-115` | v0.5.1 CLI; bins `clockify115` and `clk115`; command metadata is generated into the product surface; `--output table\|json\|ndjson`/`--compact`/`--select` controls |
| `mcp/` | `@apet97/clockify-mcp-115` | v0.8.1 stdio MCP; bin `clockify115-mcp`; tool/resource counts are generated into the product surface |
