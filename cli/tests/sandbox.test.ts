/**
 * Live sandbox tests for @clockify/cli. Each test invokes `main()`
 * (the same entrypoint the `clockify` / `clk` bin uses) with --json
 ...
 * command group, plus `clockify status`. CRUD round-trips through the
 * CLI surface (e.g. `clk webhooks create` → `clk webhooks delete`)
...
describeLive("@clockify/cli live sandbox", () => {
    let logged: string[] = [];
    let errored: string[] = [];
    ...
    async function runCli(...args: string[]): Promise<{ code: number; json: unknown }> {
        ...
        const code = await main(["node", "clk", "--json", ...args]);
        if (code !== 0) {
            throw new Error(
    ...
    it("clk status returns workspace + user info", async () => {
    ...
    it("clk tags list returns an array (existing surface smoke)", async () => {
        const { json } = await runCli("tags", "list", "--limit", "5");
        expect(Array.isArray(json)).toBe(true);
    }, 20_000);

    it("clk projects list returns an array (existing surface smoke)", async () => {
        const { json } = await runCli("projects", "list", "--limit", "5");
        expect(Array.isArray(json)).toBe(true);
    }, 20_000);

    it("clk clients list returns an array (existing surface smoke)", async () => {
        const { json } = await runCli("clients", "list", "--limit", "5");
        expect(Array.isArray(json)).toBe(true);
    }, 20_000);

    it("clk webhooks list returns an array (new in v0.2 surface)", async () => {
        const { json } = await runCli("webhooks", "list");
        expect(Array.isArray(json)).toBe(true);
    }, 20_000);

    it("clk invoices list returns an array (new in v0.2 surface)", async () => {
    ...
    it("clk expenses list returns an array (new in v0.2 surface)", async () => {
        const { json } = await runCli("expenses", "list", "--limit", "5");
        expect(Array.isArray(json)).toBe(true);
    }, 20_000);

    it("clk audit-log search accepts the documented filter shape", async () => {
